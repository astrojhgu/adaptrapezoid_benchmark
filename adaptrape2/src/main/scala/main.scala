...
class Interval[T: Fractional : Ordering](val x1: T, val f1: T, val x2: T, val f2: T) {
  ...
  * */
  ...
  def try_refine(func: T => T, eps: T, full_width: T): Either[(Interval[T], Interval[T]), (Interval[T], Interval[T])] = {
    /*
...
object Interval {
  final def refine_iter[T: Fractional : Ordering](func: T => T, previous: (List[Interval[T]], List[T]), eps: T, full_width: T): (List[Interval[T]], List[T]) = {
    /*
    ...
    * */
    ...
    for (i <- previous._1) {
      i.try_refine(func, eps, full_width) match {
        ...
        case Right((i1, i2)) => {
          refined = i1.subsum::i2.subsum :: refined
        }
      }
    }
  ...
  final def refine_until_converge[T: Fractional : Ordering](func: T => T, init_ticks: List[T], eps: T): T = {
    ...
    var s = 0.0;
    refined.sortBy(x => Fractional[T].abs(x)).foldLeft(Fractional[T].zero) { (a, b) => a + b }
    //refined.foldLeft(Fractional[T].zero) { (a, b) => a + b }
  }
}
